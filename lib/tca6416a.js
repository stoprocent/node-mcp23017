...
var TCA6416A = (function() {
  ...
  TCA6416A.prototype.dirAPullUpState = 0x00; //initial state of GPIO A pull up resistor state
  TCA6416A.prototype.dirBPullUpState = 0x00; //initial state of GPIO B pull up resistor state
  ...
  */
  ...
  TCA6416A.prototype.digitalReadSync = function (pin) {
    var register = pin >= 8 ? WRITE_GPIOB_ADDR : WRITE_GPIOA_ADDR; //get the register to read from
    pin = pin >= 8 ? pin - 8 : pin; //remap the pin to internal value
    var pinHexMask = Math.pow(2, pin); //create a hexMask
  ...
  TCA6416A.prototype._sendSync = function (cmd, values) {
    this.log(`send ${Buffer.from([cmd]).toString("hex")}, ${values[0].toString(2).padStart(8, "0")}`)
    this.wire.writeByteSync(this.address, cmd, values[0]);
  };

  TCA6416A.prototype._readSync = function (cmd, callback) {
    let value = this.wire.readByteSync(this.address, cmd);
    this.log(`read ${Buffer.from([cmd]).toString("hex")}, ${value.toString(2).padStart(8, "0")}`)
    return value
  };
